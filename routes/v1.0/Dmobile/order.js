...
router.get("/queryall",function(req,res,next){
    ...
    console.log(userId)
    DB.queryDB("select  order_number,user_address,order_size,order_status,user_reserve_time,driver_name,driver_phone,driver_reach_trash from v_assign_order where driver_id = (select driver_id from t_driver_list where wechat_id = 10 and driver_is_deleted = 0 limit 0,1)",userId,function(error,result,next){
        if (error) {
            let responseJson = {
                code: 20002,
                message: '查询失败',
    ...
    let {orderId,getImageList} = req.body 
        ...
        [JSON.stringify(getImageList),orderId],function(error,result,fields){
            if (error) {
                ...
                // 装车完成订单状态更新为5,运输中
                DB.queryDB("update  t_order_list set order_status = 5 where  order_id = ?",orderId,function(error,result,fields){
                    if (error) {
                        let responseJson = {
                            code: 20002,
                            message: '更新时间失败',
...
router.post("/update/complete",function(req,res,next){
    let {orderId,completeImageList} = req.body 
        DB.queryDB("update t_order_info_list set driver_complete_img = ? , driver_complete_time = now() where  order_id = ?",
        [JSON.stringify(completeImageList),orderId],function(error,result,fields){
            if (error) {
                let responseJson = {
                    code: 20002,
                    message: '更新时间失败',
                ...
                // 装车完成订单状态更新为5,运输中
                DB.queryDB("update  t_order_list set order_status = 6 where  order_id = ?",orderId,function(error,result,fields){
                    if (error) {
                        let responseJson = {
                            code: 20002,
                            message: '更新时间失败',
