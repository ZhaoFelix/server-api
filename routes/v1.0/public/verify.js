...
let smsClient = new SMSClient({
  ...
  secretAccessKey: ENV.ali.secretAccessKey
})
  ...
  smsClient
    ...
    .then(
      function (re) {
        ...
        // 发送成功后将验证码存入
        DB.queryDB(
          'INSERT  INTO `t_phone_code` (`phone_number`,`phone_code`,`phone_created_time`,`phone_deadline`)  VALUES(?,?,NOW(),DATE_ADD(NOW(),INTERVAL 10 MINUTE))',
          [phone, code],
...
router.post('/check', function (req, res, next) {
  ...
  DB.queryDB(
    'select * from `t_phone_code` where phone_number = ? and phone_is_deleted = 0 order by phone_created_time desc limit 1',
    [phone],
    function (error, result, fields) {
      if (error) {
        let responseJson = {
...
router.post('/checked', function (req, res, next) {
  ...
  return new Promise((resolve, reject) => {
    //根据手机号查询
    DB.queryDB(
      'select estate_id from t_estate_list where estate_phone = ? and estate_is_deleted = 0 and estate_is_auth = 0',
      phone,
      function (error, result, fields) {
        if (error) {
...
router.post('/auth', function (req, res, next) {
  ...
  return new Promise((resolve, reject) => {
    DB.queryDB(
      'update  t_user_list set  user_type = 1 where  user_id = ?',
      userId,
      function (error, result, fields) {
        if (error) {
    ...
    .then((data) => {
      ...
      return new Promise((resolve, reject) => {
        DB.queryDB(
          'update  t_estate_list set  estate_is_auth = 1,wechat_id = ?, estate_wechat_time = NOW() where  estate_is_deleted = 0  and estate_is_auth = 0 and estate_phone = ?',
          [userId, phone],
          function (error, result, fields) {
            if (error) {
