...
router.get('/query/all', function(req, res, next) {
  ...
  DB.queryDB(
    'SELECT user_id, wechat_open_id, wechat_nickname,wechat_region,wechat_avatar,wechat_last_time, wechat_phone,user_type,wechat_created_time FROM t_user_list WHERE wechat_is_deleted = 0 limit ? offset ?',[limit,offset],
    function(error, result, fields) {
      if(error) {
        ...
        // 查询记录总数
        DB.queryDB("select count(user_id) as total FROM t_user_list WHERE wechat_is_deleted = 0",function(error,resu,fields){
          if (error) {
            let respondJson = {
              code: 20002,
              message: '查询记录条数失败',
